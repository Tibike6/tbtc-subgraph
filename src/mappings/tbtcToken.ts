...
export function handleTransfer(event: TranferEvent): void {
  ...
  if (dailyTransferAggregate == null) {
    ...
    dailyTransferAggregate.count = dailyTransferAggregate.count.plus(
      BigInt.fromI32(1)
    );
    dailyTransferAggregate.sum = dailyTransferAggregate.sum.plus(
      transfer.value
    );
  }
  dailyTransferAggregate.save();

  let montlyTransferDate = dayMonthYearFromTimestamp(transfer.timestamp!).slice(
    0,
    7
  );
  let montlyTransferAggregate = MontlyTransferAggregation.load(
    montlyTransferDate
  );
  if (montlyTransferAggregate == null) {
    ...
    montlyTransferAggregate.count = montlyTransferAggregate.count.plus(
      BigInt.fromI32(1)
    );
    montlyTransferAggregate.sum = montlyTransferAggregate.sum.plus(
      transfer.value
    );
  }
